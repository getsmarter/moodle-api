...
module Moodle::Api
  RSpec.describe Client do
    let(:params) { { 'criteria[0][key]' => 'firstname', 'criteria[0][value]' => 'Jon' } }

    describe 'configuration setup' do
      it 'allows configuration via options' do
        client = Moodle::Api::Client.new({host: 'host', token: 'token'})
    ...
    context 'valid configuration' do
      let(:client) { Moodle::Api::Client.new(host: 'http://dev.vle.getsmarter.co.za', token: '072556801bf07076fff6bff2a463b7c5') }
      ...
      it 'raises moodle error if incorrect method name is used' do
        VCR.use_cassette('external_service/invalid_service') do
          ...
          }.to raise_error(Moodle::Api::MoodleError, "Can not find data record in database table external_functions.")
        end
      end
      ...
      it 'raises moodle error if trying to access method that has not been made external' do
        VCR.use_cassette('external_service/valid_service_not_external') do
          ...
          }.to raise_error(Moodle::Api::MoodleError, "Access control exception")
        end
      end
    end

    context 'invalid configuration' do
      let(:client) { Moodle::Api::Client.new(host: 'http://dev.vle.getsmarter.co.za', token: 'invalidtoken') }

      it 'raises moodle error if token is invalid' do
        VCR.use_cassette('external_service/valid_service_with_invalid_token') do
          expect {
            client.make_request(:core_user_get_users, params)
          }.to raise_error(Moodle::Api::MoodleError, "Invalid token - token not found")
        end
      end
    end
  end
